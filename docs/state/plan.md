...
**Статус: принят PO на минималках** (2026-08-19). Сковородка только с корня крышки (☰ + жёлоб 20pt на `LidScreen`). `PanSlideLayout`: закрытая крышка = весь экран; открытая — карточка, скругление экрана. «Facio» в ленте с `pagePadding`. Шторка ниже статус-бара (`deviceSafeArea`). Use/Inspect: левый край = назад. Talks — архив в сковородке. Deeds → кебаб. Inspect ≠ Use. Плюс создаёт случай. Кебаб урезан (PO ок); зажатие плитки = кебаб. Настройки — подпись. Док рта на Inspect нет.

**Сразу после акцепта, не шаг 6.** Рефактор костылей `apps/mobile-swiftui` без смены принятого поведения, затем `/full-review`. Хвост: `EmptyView` в `principal`; `compositingGroup` на весь стек крышки; UIKit `keyWindow` для угла / insets / ширины шторки; `PressGate`; `NotificationCenter` клавиатуры в `PanHost`; двойная атмосфера (`PanHost` + `containerBackground`). Не воскрешать `HStack`+offset, `safeAreaPadding(.leading)`, `if revealed > 0` на dim, `zIndex` шторки выше крышки, `GeometryReader` вокруг стека.
