...
Правило: не прыгать через шаг без явной причины. Шаги 0–5 приняты (5 на минималках). Дальше — волны **В1–В3** (принят PO, 2026-08-20): рот → неделя/пауза локально → база и облако. В работе: **В1.0**. RFC не переписывать.
...
Принят PO 2026-08-20. Ведёт **PM**; субагенты — разработчики, скилы по `AGENTS.md`. В работе: **В1.0**. Не прыгать в В1.1+ и не в В2/В3.
...
**Статус: принят PO в составе волн (2026-08-20). В работе.**
...
**Не входит.** Рантаймы этих типов. UI. MCP.
...
**Не входит.** Заморозка на дни. Сетка 7 дней. Live CI. Новые имена. Countdown.
...
**Входит.** MCP в том же процессе; вызов = `apply_tool`; стол = снимок хода. Тест: `list_desk` + `add_cue` совпадают с прямым apply. Compose — один `talk`.

**Не входит.** База. Auth. Замена HTTP телефона. `search_*`. Календарные tools.
