...
**Статус: в коде, не принят** (2026-08-19). PO ещё замечает UI. Сковородка только с корня крышки (☰ + жёлоб 20pt на `LidScreen`). Крышка — полная высота, скругление экрана, «Facio» не липнет. Use/Inspect: левый край = назад. Talks — архив в сковородке. Deeds → кебаб. Inspect ≠ Use. Плюс создаёт случай. Кебаб урезан (PO ок); зажатие плитки = кебаб. Настройки — подпись. Док рта на Inspect нет.
