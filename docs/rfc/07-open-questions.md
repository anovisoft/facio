...
| Q34 | Several occurrences and several hours inside one period | **Decided 2026-08-30 (PO), from a real request:** *"remind me every day at 10, 12, 15, 16:30, 18, 21, 22 to check Upwork, and a checkbox for each check."* The desk could express neither half — `window` holds one `latest_by`, so the second hour overwrote the first, and the projection places at most one occurrence per date, so the second check of the day had nothing to tick. Both are now in scope. **This does not reopen Q26:** a cadence is still a count per period, never fixed weekdays. What changes is *inside* the period — a subject may promise N occurrences in one day and may fire at N stated hours. **Not** a calendar: the hours belong to the subject's window, they are not events with their own life, and nothing here invents an hour the person did not say. **Refined 2026-08-30 after the first day on a phone:** N occurrences are N in the law and **one tile on the lid**, grouped by `group_id` — seven anonymous squares are unreadable, you cannot tell which one is the 15:00 check, and they crowd out everything else. The tile borrows the stepper's compactness (the next hour large, `3/7`, a row of marks) and refuses its **pointer**: marks are independent and unordered, because a pointer cannot record «did 10 and 15, missed 12», and the miss is the product. A stepper stays what it is — beats inside one occurrence. |
