...
Where the options come from: they are **layouts of `count` over the period**, not lucky days. Given the count and the days the period still has free, the row offers the distinct *spacings* — the tight run, the even spread, and one that leans on the weekend — capped at three by the rule above. No layout is offered that the period cannot hold, and days already spent in this period keep what they hold. The layout itself is computed on the ISO week (Monday first) and is the same in every language — a spacing is a spacing. What the locale supplies is the **names**, and how the row is read; a row that reordered itself per locale would no longer be the same placement. Whether a Sunday-first week should spread differently is a separate question and not decided here.
