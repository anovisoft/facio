/**
 * Focus Engine v0 (Facio 0.1 Slice B) — ranking truth for Continue.
 *
 * D1 default only (13-continuity): overdue > last day > short ≤5 min > others.
 * No pin, no ML, no time-of-day / streak weights.
 *
 * Predicates (deterministic, explainable tiers — not opaque scores):
 *
 * 1. Overdue — has executable next_action whose day_offset is strictly behind
 *    unlocked_day_index (catch-up debt: pending work from an earlier day while
 *    the unlock window has moved forward).
 * 2. Last day — unlocked_day_index >= cycle.horizon_days - 1 (current unlock is
 *    the last day of the cycle).
 * 3. Short ≤5m — next_action.estimate_min != null && estimate_min <= 5.
 * 4. Others — remaining active Guides with next_action.
 * 5. Waiting — active Guides without next_action (peek-only / day done early);
 *    always after executable candidates.
 *
...
function tierOf(p: ProjectSummary): Tier {
  if (p.next_action == null) return TIER_WAITING;
...
/**
 * Rank active Continue Sessions. Optional `now` reserved for future day logic;
