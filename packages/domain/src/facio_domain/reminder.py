...
def window_from_closing(closes_at: time) -> Window:
    ...
    return Window(latest_by=latest, closes_at=closes_at)


def reminder_fire_at(subject: Subject, window: Window, on_date: date) -> datetime:
    """When the reminder fires on `on_date`. The window sets the hour."""
    del subject
    return datetime.combine(on_date, window.latest_by)
